# Single Source of Truth — Architecture Spec Prompt

You are maintaining architecture specifications organized as **one file per concept**. Every distinct architectural element has exactly one canonical Markdown file. No monolithic documents. No duplicated definitions.
...
Adapt the section folders to the project's needs. A common starting point:

```
[architecture-root]/
├── README.md                          ← root navigation table
├── 01-introduction-and-goals/
├── 02-constraints/
├── 03-context-and-scope/
├── 04-solution-strategy/
├── 05-building-block-view/            ← components, subsystems, interfaces
├── 06-runtime-view/                   ← request flows, execution sequences
├── 07-deployment-view/                ← infrastructure, GitOps, config management
├── 08-crosscutting-concepts/          ← security, observability, settings, logging, errors
├── 09-architecture-decisions/         ← ADR-NNN format
...
If the project already has a folder structure, use it. The principles apply regardless of how sections are named or organized.
...
**Bad headings (numbered — will break when reordered):**
```markdown
## 1. Overview
### 2.1 Components
## 3. Error Handling
...
#### Bad: prose with no link at all

```markdown
Errors are classified into three tiers (see Error Handling spec, section on classification).
```
"Section on classification" is not a link. The reader has to find it manually.
...
The architecture must carry its own maintenance rules at the entry point — the root `README.md` of the architecture directory. This is the first file every agent and human reads. If the rules live here, they are in context before any edit happens, regardless of which tool or agent is used.

After the navigation table in the root `README.md`, add or maintain a `## Maintenance Rules` section containing:

1. The rules summary (all 12 rules below)
2. The file format template (with summary block, `Depends on`, `Key sections`)
3. The "When Creating New Specs" checklist
4. The "When Reviewing Existing Specs" checklist

This is the operational extract — not the full prompt. The full prompt with examples, good/bad comparisons, and detailed rationale lives separately and is linked from the README for edge cases.

The Maintenance Rules section in the README should look like:

```markdown
## Maintenance Rules

These rules govern how spec files in this directory are created, modified, and linked. Read before editing any file.

> Full reference with examples and rationale: [single-source-of-truth.md](path/to/single-source-of-truth.md)
...
[Include the file format template from this prompt]

### Creating New Specs

[Include the "When Creating New Specs" checklist from this prompt]

### Reviewing Existing Specs

[Include the "When Reviewing Existing Specs" checklist from this prompt]
```
...
This makes the architecture self-policing: the rules travel with the specs, are version-controlled alongside them, and are in every agent's context at the moment it matters.

### Rules Summary

1. **One concept = one file.** No exceptions without explicit justification.
2. **No duplication.** If it exists in two places, one dies and becomes a section-specific link.
3. **Summary block on every file.** Summary paragraph, `Depends on` links, `Key sections` list — before the `---`. A reader decides whether to go deeper from this block alone.
4. **Section-level links.** Every cross-reference is a markdown link to the exact heading (`file.md#section`), not just the file. Whole-file links only for general concept references.
5. **Links, not prose.** References are markdown links — not "see X for Y" sentences. The link is the reference.
6. **No numbered headings.** Headings are descriptive names only (`## Error Classification`, not `## 3. Error Classification`). Numbers create renumbering churn and break anchors when sections are added or reordered.
7. **Stable anchors.** Headings are link targets. Never rename a heading without updating all inbound links.
8. **≤ 500 lines.** Split by sub-concern when exceeded.
9. **Provenance comment.** Every file tracks where its content came from.
10. **Indexes stay current.** Section README.md reflects actual files.
11. **Relative links only.** No absolute paths, no duplicated content as "convenience copies."
12. **kebab-case filenames.** Descriptive, unique, lowercase.
...
Apply these rules to the files or task I provide next: @specs/v2.1-architecture
